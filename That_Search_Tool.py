...
#Added by Ajay on 10 Feb
#ITEMS_VIEW = "inv_item_master"
...
def _safe_url_len(prepped):
    return len(prepped.url or "")

def _shorten(s, keep=240):
    if s is None:
        return ""
    s = str(s)
    return s if len(s) <= keep else f"{s[:keep]}... (len={len(s)})"
...
def _send_and_normalize(sess, prepped, original_req, timeout, max_retries):
    for attempt in range(max_retries):
        try:
            ...
            return normalize_keys(df if isinstance(df, pd.DataFrame) else pd.DataFrame())
        except requests.exceptions.ConnectionError as e:
            if attempt == 0 and ("RemoteDisconnected" in str(e) or "closed connection" in str(e)):
                try:
                    global _D_SESSION
                    _D_SESSION = None
                    sess = _denodo_session()
                    prepped = sess.prepare_request(original_req)
                    continue
                except Exception:
                    pass
            ...
            try:
                ...
                body = _shorten(getattr(e.response, "text", ""), keep=240)
            except Exception:
                status = None
                body = ""
            transient = (
            ...
            if transient and attempt < max_retries - 1:
                time.sleep((2**attempt) + random.random())
                continue
            short_url = _shorten(prepped.url, keep=240)
            raise RuntimeError(_with_tip(
                f"Denodo request failed: HTTP {status}\nURL: {short_url}\nBody: {body}"
            )) from e
            ...
            if attempt < max_retries - 1:
                ...
                continue
            short_url = _shorten(prepped.url, keep=240)
            raise RuntimeError(_with_tip(f"Denodo request error: {e}\nURL: {short_url}")) from e

def _detect_chunk_param(base_params, preferred_candidates=("item_id","sku","ids","item","model","part","material")):
    def to_list_like(v):
        if isinstance(v, (list, tuple, set)):
            lst = list(map(str, v))
            return lst if len(lst) > 1 else None
    ...
    for k in preferred_candidates:
        if k in base_params:
            lst = to_list_like(base_params[k])
            if lst:
                return k, lst

    # fallback: longest list-like value
    best_k, best_list, best_len = None, None, -1
    for k, v in base_params.items():
        lst = to_list_like(v)
        if lst:
            L = len(",".join(lst))
            if L > best_len:
                best_k, best_list, best_len = k, lst, L
    return best_k, best_list

def _sql_quote(v: str) -> str:
    """SQL-quote; preserves leading zeros."""
    s = str(v).replace("'", "''")
    return f"'{s}'"

def _strip_field_predicates_from_filter(filter_text: str, fields: list[str]) -> str:
    """
    Remove predicates on any of the given fields from a $filter string.
    Handles common patterns (case-insensitive):
      ...
      - field LIKE '...'
      - ( ... ) wrappers
    Then cleans up dangling AND/OR and extra whitespace/parentheses.
    """
    if not filter_text:
        return filter_text
    ...
    def _mk_patterns(field_name: str):
        # Accept both UPPER(field) and the raw field
        field_re = rf"(?:UPPER\s*\(\s*{re.escape(field_name)}\s*\)|{re.escape(field_name)})"
        return [
            rf"\(\s*{field_re}\s*=\s*'[^']*'\s*\)",      # (field = 'x')
            rf"{field_re}\s*=\s*'[^']*'",                # field = 'x'
            rf"{field_re}\s+IN\s*\(\s*'[^)]*'\s*\)",     # field IN ('a','b',...)
            rf"{field_re}\s+LIKE\s*'[^']*'",             # field LIKE 'x%'
        ]

    # Remove matches for each field variant
    for field in fields:
        for p in _mk_patterns(field):
            f = re.sub(p, "", f, flags=re.IGNORECASE)

    # Remove redundant connectors left behind
    f = re.sub(r"\bAND\b\s*(\)|$)", r"\1", f, flags=re.IGNORECASE)
    f = re.sub(r"\bOR\b\s*(\)|$)", r"\1", f, flags=re.IGNORECASE)
    f = re.sub(r"(\(|^)\s*\bAND\b\s*", r"\1", f, flags=re.IGNORECASE)
    f = re.sub(r"(\(|^)\s*\bOR\b\s*", r"\1", f, flags=re.IGNORECASE)

    # Collapse spaces and clean empty parens
...
def denodo_fetch_all_safe(
    ...
    *,
    ...
    url_len_limit=7000,    # infra-friendly limit
    list_param_key="item_id",   # your big IN(...) param
    filter_field="item_id",     # unqualified name as exposed by the view
    filter_field_qualified="IM.item_id",  # qualified per your SQL
    chunk_size=200,        # adjust as needed
    dedupe=True,
    dedupe_keys=None
):
    """
    Robust GET against Denodo with auto-chunking and alias-aware $filter merge:
      - chunks the list under `item_id`
      - strips any existing item_id predicates (qualified or unqualified) from $filter
      - injects "... AND <field> IN (...)" using the alias form if the $filter already uses "IM."
    """
    sess = _denodo_session()
    base_params = _stringify_params(params or {})

    # Try single-shot first
    req = requests.Request("GET", url, headers=headers, params=base_params)
    prepped = sess.prepare_request(req)
    if _safe_url_len(prepped) <= url_len_limit:
        return _send_and_normalize(sess, prepped, req, timeout, max_retries)

    # Identify the list to chunk
    candidate_key = list_param_key if list_param_key in base_params else None
    candidate_vals = None
    ...
    if candidate_key:
        candidate_vals = _to_list(base_params[candidate_key])
        if len(candidate_vals) <= 1:
            candidate_key, candidate_vals = None, None

    if not candidate_key:
        candidate_key, candidate_vals = _detect_chunk_param(base_params)

    if not candidate_key or not candidate_vals:
        longest = sorted(((k, len(str(v))) for k, v in base_params.items()), key=lambda x: -x[1])[:5]
        longest_hint = ", ".join([f"{k}({l})" for k,l in longest])
        raise RuntimeError(_with_tip(
            "Query URL exceeds limit and no chunkable param found.\n"
            f"Tried preferred key '{list_param_key}' and auto-detect.\n"
            f"Top long params: {longest_hint or 'n/a'}\n"
            "Pass list_param_key='<your_param>' or provide the big list as a Python list/comma string."
        ))

    # We will ALWAYS merge into $filter (to avoid duplicate param predicates)
    # Decide whether to use qualified field name based on the existing $filter
    base_filter = base_params.get("$filter", "") or ""
    use_qualified = ("IM." in base_filter) or (re.search(r"\bIM\.", base_filter) is not None)
    inject_field = filter_field_qualified if use_qualified else filter_field

    dfs = []
    current_chunk_size = max(5, int(chunk_size))
    while True:
        try:
            for chunk in _iter_chunks(candidate_vals, current_chunk_size):
                chunk_params = dict(base_params)

                # Remove the separate item_id param to avoid duplicates
                chunk_params.pop(candidate_key, None)

                # Strip any existing item_id predicates (qualified or unqualified)
                cleaned_filter = _strip_field_predicates_from_filter(
                    base_filter,
                    fields=[filter_field, filter_field_qualified]
                )

                # Build IN(...) with proper quoting to preserve leading zeros
                in_list = ", ".join(_sql_quote(x) for x in chunk)
                in_pred = f"{inject_field} IN ({in_list})"

                # Compose the new $filter
                if cleaned_filter.strip():
                    new_filter = f"({cleaned_filter}) AND {in_pred}"
                else:
                    new_filter = in_pred
                chunk_params["$filter"] = new_filter

                # Prepare and enforce URL limit
                req = requests.Request("GET", url, headers=headers, params=chunk_params)
                prepped = sess.prepare_request(req)
                if _safe_url_len(prepped) > url_len_limit:
                    raise ValueError("chunk_too_large")

                df_part = _send_and_normalize(sess, prepped, req, timeout, max_retries)
                if isinstance(df_part, pd.DataFrame) and not df_part.empty:
                    dfs.append(df_part)
            break
        except ValueError as ve:
            if str(ve) == "chunk_too_large":
                if current_chunk_size <= 5:
                    raise RuntimeError(_with_tip(
                        "Even tiny chunks exceed URL limit after filter merge. "
                        "Consider POST or reduce per-chunk filters."
                    ))
    ...
    if dedupe:
        out = out.drop_duplicates(subset=dedupe_keys, keep="first") if dedupe_keys else out.drop_duplicates()
    return out

def denodo_fetch_all(url, headers, params):
    # legacy alias; keep for backward-compat
    # hard-wired for item_id chunking merged into $filter
    return denodo_fetch_all_safe(
        url,
        headers,
        params,
        list_param_key="item_id",
        filter_field="item_id",  # merge chunk into $filter AND remove the separate param
        url_len_limit=7000,
        chunk_size=200,
        dedupe=True,
        dedupe_keys=["item_id"]
...
#Added by Ajay on 11 Feb 2026
# Pre-compile the null-like token pattern once
_NULL_TOKEN_PATTERN = re.compile(r"(?i)^\s*(?:<\s*null\s*>|null|none|nan)\s*$")

def normalize_all_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize a DataFrame such that:
      - ONLY text-like columns (object/string dtype) are cleaned.
      - In those columns:
          * True nulls (None, NaN, pd.NA) -> "".
          * String tokens like "<null>", "null", "None", "NaN" (any case, with spaces) -> "".
          * Strings are whitespace-trimmed.
      - Non-text columns (numeric, datetime, boolean, categorical, etc.) are preserved, including their nulls.

    Returns a new DataFrame; original is not modified.
    """
    out = df.copy()

    # Identify columns that are text-like
    text_cols = out.select_dtypes(include=["object", "string"]).columns
    if len(text_cols) == 0:
        return out

    def _clean_cell(x):
        # Null-like (None, NaN, pd.NA, NaT)
        if pd.isna(x):
            return ""
        # If it's a string, trim and check for tokens
        if isinstance(x, str):
            s = x.strip()
            return "" if _NULL_TOKEN_PATTERN.fullmatch(s) else s
        # If it's not a string (e.g., dict/list/number in an object col), keep as-is
        return x

    # Apply per-cell cleaner to only the text-like columns
    out[text_cols] = out[text_cols].applymap(_clean_cell)
...
#Updated by Ajay Jadhav on 15 Jan 2026
#    if or_wild:
#       where += [
...
#       ]
    if or_wild:
        where += [
            f"PI.item_field_c30_b LIKE '{esc(t).upper()}'"
            for t in or_wild
            if t
        ]
...
def _fetch_inventory_chunked(ids_for_inv, headers, chunk_size=150):
    """
    Fetch inventory rows for a list of item IDs.

    Uses denodo_fetch_all_safe auto-chunking by passing the list as a param
    (inventory_item_id), letting it merge an IN(...) predicate into $filter.
    We deliberately omit $select to avoid 400s on unknown fields (e.g., 'location').
    """
    if not ids_for_inv:
        return pd.DataFrame()

    ids_for_inv = [str(x).strip() for x in ids_for_inv if str(x).strip()]

    inv_params = {
        # No $select on purpose to avoid unknown-field errors.
        "inventory_item_id": ids_for_inv,
    }

    df_inv = denodo_fetch_all_safe(
        f"{BASE_URL}/{INV_VIEW}",
        headers,
        inv_params,
        list_param_key="inventory_item_id",
        filter_field="inventory_item_id",
        filter_field_qualified="IM.inventory_item_id",
        chunk_size=chunk_size,
        dedupe=True,
        dedupe_keys=["inventory_item_id"],
    )

    return normalize_keys(df_inv if isinstance(df_inv, pd.DataFrame) else pd.DataFrame())


def _fetch_inventory_chunked_old(ids_for_inv, headers, chunk_size=150):
    frames = []
...
def items_lookup(
    ...
    # repurposed for Q-codes (AND across terms)
    q_and_terms = [x.strip() for x in (or_wildcards or []) if x and x.strip()]
    ...
    # IMPORTANT: Do NOT inject long item_id IN(...) here; pass as a param later.
    im_where = build_itemmaster_filter(
        item_ids=[],           # leave empty; we pass item_ids separately
        mfg_parts=mfg_parts,
        and_wild=and_wild,
        either_or=either_or,
        or_wild=None,
    ...
    if q_and_terms:
        ...
        # contains-match for each Q-term ANDed together
        ...
        if df_ctrl_prefilter.empty or "inv_item_id" not in df_ctrl_prefilter.columns:
            # If Q-codes are required and none match, return empty
            return (
                (
        ...
        # If caller also supplied explicit item_ids, INTERSECT with Q-code matches
        if item_ids:
            s = set(x.strip().upper() for x in ctrl_ids)
    ...
    # ✅ FINAL ID SET FOR ITEM_MASTER:
    # - If user specified item_ids, use those (already intersected with ctrl_ids if needed).
    # - Else if no item_ids but we have Q-codes, use ctrl_ids.
    # - Else (no ids, no q-codes) rely on other filters in im_where (could be wide).
    if item_ids:
        ids_for_im = [str(x).strip() for x in item_ids if str(x).strip()]
    elif ctrl_ids:
        ids_for_im = [str(x).strip() for x in ctrl_ids if str(x).strip()]
    else:
        ids_for_im = []

    if ids_for_im:
        im_params["item_id"] = ids_for_im  # let denodo_fetch_all_safe auto-chunk & merge

    df_items = denodo_fetch_all_safe(
        f"{BASE_URL}/{ITEMS_VIEW}", headers, im_params,
        list_param_key="item_id",            # key to chunk
        filter_field="item_id",              # injects into $filter
        filter_field_qualified="IM.item_id", # uses qualified if $filter has 'IM.'
        chunk_size=200,
        dedupe=True,
        dedupe_keys=["item_id"],
    )
    df_items = normalize_keys(df_items if isinstance(df_items, pd.DataFrame) else pd.DataFrame())
    ...
    # ---- 4) fetch INVENTORY (always drive off what ITEM_MASTER returned) ----
    df_inv_raw = _fetch_inventory_chunked(ids_from_items, headers, chunk_size=150)
    df_inv_raw = normalize_keys(df_inv_raw if isinstance(df_inv_raw, pd.DataFrame) else pd.DataFrame())
    df_inv_agg = aggregate_inventory(df_inv_raw)  # computes reserved & available

    # ---- 5) fetch CONTROLS for the same final IDs ----
    if q_and_terms:
        # If we already have the prefilter, limit it to the final IDs
        df_ctrl = df_ctrl_prefilter[df_ctrl_prefilter["inv_item_id"].astype(str).str.upper().isin(ids_from_items)].copy()
    else:
        # Or fetch controls for the final IDs
        ctrl_params = {
            "$select": "inv_item_id,item_field_c30_b,item_field_c10_c",
            "inv_item_id": [str(x) for x in ids_from_items],
        }
        df_ctrl = denodo_fetch_all_safe(
            f"{BASE_URL}/{ITEM_CONTROLS}", headers, ctrl_params,
            list_param_key="inv_item_id",
            filter_field="inv_item_id",
            filter_field_qualified="IM.inv_item_id",
            chunk_size=200,
            dedupe=True,
            dedupe_keys=["inv_item_id"],
        )
        df_ctrl = normalize_keys(df_ctrl if isinstance(df_ctrl, pd.DataFrame) else pd.DataFrame())
    ...
    if filter_positive_only and "available_quantity" in df.columns:
        df = df[df["available_quantity"].fillna(0) >= 0]  # keep as you set (>= 0)
    ...
    out = df.rename(
        columns={
            ...
            # If normalize_keys didn't already map these:
            "item_field_c30_b": "q_codes",
            "item_field_c10_c": "ctrl_in",
        }
    )
...
class DenodoQuery(QWidget):
    ...
    def display_data(self):
        ...
        bom_on = bool(getattr(self, "bom_mode", None)
                      and self.bom_mode.isChecked())
        ...
        if hasattr(self, "chk_include_unassigned"):
            try:
                include_unassigned = bool(
                    self.chk_include_unassigned.isChecked())
            except Exception:
                include_unassigned = True
        ...
        reserve_across_bom = bool(getattr(self, "reserve_across_bom_chk", None)
        ...
        if bom_on:
            ...
            either_or = [[e.strip() for e in g.split(",") if e.strip()]
                         for g in eo_raw] if eo_raw else []
        ...
        try:
            result = items_lookup(
                ...
                mfg_part=(_parse_list(_safe_line_text("mfg_part"))
                          if not bom_on else []),
        ...
        target_currency = (
            getattr(self, "target_currency", "USD") or "USD").upper()
    ...
    # =========================
    # Local/PDM Item-ID resolver (ADD-ONLY)
    # =========================
    import os
    import sys
    import json
    ...
    # Default engineering file extensions to consider (tweak per environment if needed)
    _DEFAULT_EXTS = {
        ".sldprt", ".sldasm", ".slddrw", ".step", ".stp", ".iges", ".igs",
        ".dwg", ".dxf", ".pdf", ".xlsx", ".xls", ".csv", ".txt"
    }

    # Environment variables to configure roots (semicolon-separated paths)
    _ENV_LOCAL_PATHS = "LOCAL_ITEM_SEARCH_PATHS"
    _ENV_PDM_PATHS = "PDM_VAULT_PATHS"

    # Simple cache to speed up repeated scans
    _CACHE_PATH = os.path.expanduser("~/.pdm_item_search_cache.json")
    _CACHE_TTL_SECONDS = 180  # 3 minutes


    def _read_text_widget_safe(self, name: str) -> str:
        """Safe read of either QLineEdit or QTextEdit without coupling to Qt types."""
        try:
    ...
    def _pn_tokens_from_ui(self, bom_on: bool) -> List[str]:
        """
        Collect Part Number tokens from UI (BOM or single-entry fields).
        Mirrors your existing token logic to keep behavior consistent.
        """
        if bom_on:
            raw = _read_text_widget_safe(self, "part_no_edit") or _read_text_widget_safe(self, "item_id")
            toks = [t.strip() for t in re.split(r"[,\s]+", raw or "") if t.strip()]
            return [t.upper() for t in toks]
        else:
            toks = [t.strip() for t in (_read_text_widget_safe(self, "item_id") or "").split(";") if t.strip()]
            if not toks:
                # If item_id field is empty, fall back to Part No field
                raw = _read_text_widget_safe(self, "part_no_edit")
                toks = [t.strip() for t in re.split(r"[,\s;]+", raw or "") if t.strip()]
            return [t.upper() for t in toks]


    def _collect_default_search_roots(self) -> List[str]:
        """
        Determine search roots in a conservative, configurable way.
        Priority:
        1) self.pdm_paths / self.local_search_paths (if your app sets these)
        2) Environment: PDM_VAULT_PATHS, LOCAL_ITEM_SEARCH_PATHS (semicolon-separated)
        3) Common vault mount points / local drives
        """
        roots = []
        # App-provided
        for attr in ("pdm_paths", "local_search_paths"):
            p = getattr(self, attr, None)
            if isinstance(p, (list, tuple)):
                roots.extend([str(x) for x in p if x])
            elif isinstance(p, str) and p.strip():
                roots.extend([s.strip() for s in p.split(";") if s.strip()])

        # Environment
        for var in (_ENV_PDM_PATHS, _ENV_LOCAL_PATHS):
            val = os.environ.get(var, "")
            if val.strip():
                roots.extend([s.strip() for s in val.split(";") if s.strip()])

        # Known patterns (Windows drive letters commonly used for vaults)
        if sys.platform.startswith("win"):
            try:
                import string
                for letter in string.ascii_uppercase:
                    ...
                    if os.path.exists(drive):
                        # Add plausible engineering/vault subfolders
                        roots.extend([
                            drive,
                            os.path.join(drive, "Vault"),
                            os.path.join(drive, "PDM"),
                            os.path.join(drive, "Engineering"),
                            os.path.join(drive, "Design")
                        ])
            except Exception:
                pass
            # Also check common local folders
            user_docs = os.path.join(os.path.expanduser("~"), "Documents")
            for guess in [
                user_docs,
                os.path.join(user_docs, "PDM"),
                os.path.join(user_docs, "Vault"),
                os.path.join(user_docs, "Engineering"),
            ]:
                if os.path.exists(guess):
                    roots.append(guess)
        else:
            # Linux/macOS: typical mounts or project dirs
            for guess in [
                "/mnt/pdm", "/mnt/vault", "/mnt/engineering",
                "/media", "/Volumes", str(pathlib.Path.home() / "Projects"),
            ]:
                if os.path.exists(guess):
                    roots.append(guess)

        # Deduplicate while preserving order and filter to existing paths
        seen = set()
        out = []
        for r in roots:
    ...
    def _recent_cache_hit(cache_key: str) -> Tuple[bool, List[str]]:
        cache = _load_cache()
        entry = cache.get(cache_key)
        if not entry:
            return False, []
        if time.time() - entry.get("ts", 0) <= _CACHE_TTL_SECONDS:
            return True, entry.get("results", [])
        return False, []


    def _store_cache_hit(cache_key: str, results: List[str]) -> None:
        cache = _load_cache()
        cache[cache_key] = {"ts": time.time(), "results": results}
        _save_cache(cache)


    def _iter_files(roots: Iterable[str], exts: Iterable[str], max_depth: int = 6) -> Iterable[str]:
        """
        Conservative file iterator: iterates known roots, limits depth to avoid expensive walks.
        """
        ...
        for root in roots:
            ...
            for dirpath, dirnames, filenames in os.walk(root):
                # Depth limit
                depth_parts = pathlib.Path(dirpath).relative_to(root).parts
                ...
                for fn in filenames:
                    ...
                    try:
                        ...
                        # Ignore odd filenames
                        continue


    def _match_files_for_pn(files: Iterable[str], pn: str) -> List[str]:
        """
        Match strategy:
        - exact stem equals PN (case-insensitive)
        - filename contains PN token boundaries (e.g., '_PN_' or '-PN-' etc.)
        - simple wildcard fnmatch '*PN*'
        """
        pn_upper = pn.upper()
        results = []
        for f in files:
            ...
            if stem.upper() == pn_upper:
                results.append(f)
                continue
            # token boundary match
            if re.search(rf"(?:^|[_\-\. ]){re.escape(pn_upper)}(?:$|[_\-\. ])", name.upper()):
                results.append(f)
                continue
            # fallback wildcard
            if fnmatch.fnmatch(name.upper(), f"*{pn_upper}*"):
                results.append(f)
        return results


    def _resolve_item_ids_from_local_sources(self, bom_on: bool) -> List[str]:
        """
        Resolve item_ids from local/PDM based on Part Number tokens.
        Returns list of unique item_ids (same as PN tokens that were found).
        If nothing found, returns the original PN tokens (to keep behavior graceful).
        """
        pns = _pn_tokens_from_ui(self, bom_on=bom_on)
        if not pns:
            return []

        roots = _collect_default_search_roots(self)
        if not roots:
            # No roots configured; assume Item IDs equal provided PNs.
            return list(dict.fromkeys([pn.upper() for pn in pns]))

        exts = getattr(self, "pdm_file_exts", None)
        if not isinstance(exts, (list, set, tuple)):
            exts = _DEFAULT_EXTS

        # Cache key: roots + pns + exts
        cache_key = json.dumps({"roots": roots, "pns": pns, "exts": sorted(exts)})
        hit, cached = _recent_cache_hit(cache_key)
        if hit:
            return cached

        # Build a file iterator once
        files = list(_iter_files(roots, exts))
        found_item_ids = set()
        for pn in pns:
            matched = _match_files_for_pn(files, pn)
            if matched:
                # Policy: treat PN as Item ID (1:1) when any local/PDM file matches
                found_item_ids.add(pn.upper())

        results = list(sorted(found_item_ids)) if found_item_ids else list(dict.fromkeys([pn.upper() for pn in pns]))
        # Store in cache
        _store_cache_hit(cache_key, results)
        return results


    # =========================
    # Files bridge for DataWindow (ADD-ONLY)
    # =========================
    def open_files_for_pn(self, pn: str) -> None:
        """
        Open or reveal files matching the given part number from local/PDM roots.
        Respects the same roots/exts used in resolver.
        """
        try:
            roots = _collect_default_search_roots(self)
            exts = getattr(self, "pdm_file_exts", None)
            ...
            matched = _match_files_for_pn(files, pn)
            if not matched:
                QMessageBox.information(self, "Open Files", f"No local/PDM files found for Part Number: {pn}")
                return

            # Open a limited number to avoid prompt storms
            to_open = matched[:5]
            for f in to_open:
                try:
                    if sys.platform.startswith("win"):
                    ...
                    # As a fallback, print path for logs
                    print(f"Open file: {f}")

            if len(matched) > len(to_open):
                QMessageBox.information(
                    self,
                    "Open Files",
                    f"Opened {len(to_open)} files for PN '{pn}'. "
                    f"{len(matched) - len(to_open)} more matches exist."
                )
        except Exception as e:
            try:
                QMessageBox.warning(self, "Open Files", f"Failed to open files for PN '{pn}'.\n\nDetails: {e}")
            except Exception:
                pass
    ...
    def show_sql(self):
        ...
        item_ids = [i.strip() for i in self.item_id.text().split(
            ';')] if self.item_id.text() else []
        mfg_parts = [m.strip() for m in self.mfg_part.text().split(
            ';')] if self.mfg_part.text() else []
        and_wild = [a.strip() for a in self.and_wildcards.text().split(
            ';')] if self.and_wildcards.text() else []
        either_or = [e.split(',') for e in self.either_or_wildcards.text().split(
            ';')] if self.either_or_wildcards.text() else []
        or_wild = [o.strip() for o in self.or_wildcards.text().split(
            ';')] if self.or_wildcards.text() else []
        min_qty = (int(self.qty.text())
                   if self.qty.text().strip() != '' else None)
        sql = build_equivalent_sql(
            item_ids, mfg_parts, and_wild, either_or, or_wild, list(checked_BUs), min_qty)
